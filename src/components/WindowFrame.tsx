...
export const WindowFrame: React.FC<WindowFrameProps> = ({
  ...
  return (
    <div className="w-full flex flex-col rounded-2xl overflow-hidden shadow-2xl transition-all duration-200 border border-white/10 bg-slate-950/60 backdrop-blur-md">
      ...
      {windowStyle !== 'none' && (
        <div className="flex items-center justify-between px-2.5 sm:px-3 py-2 border-b border-white/10 select-none bg-slate-900/80 gap-2 overflow-x-auto no-scrollbar">
          {/* macOS Controls Left */}
          {windowStyle === 'macos' && (
            <div className="flex items-center gap-1.5 pl-1 pr-1.5 flex-shrink-0">
          ...
          {windowStyle === 'windows' && (
            <div className="flex items-center gap-1 opacity-60 flex-shrink-0 pl-1 pr-1.5">
              <FileCode2 className="w-4 h-4 text-zinc-300" />
            </div>
          )}

          {windowStyle === 'minimal' && (
            <div className="flex items-center gap-1 flex-shrink-0 pl-1 pr-1.5">
            ...
            {tabs.map((tab) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1 rounded-lg text-xs font-mono transition-all cursor-pointer group flex-shrink-0 border ${
                    isActive
                ...
                >
                  {isEditing ? (
                    <input
                      ...
                      className="bg-transparent text-xs font-mono outline-none border-b border-indigo-400 max-w-[100px] sm:max-w-[120px]"
                    />
                  ) : (
                    <span
                      onDoubleClick={() => setEditingTabId(tab.id)}
                      className="truncate max-w-[100px] sm:max-w-[130px]"
          ...
          {windowStyle === 'windows' && (
            <div className="hidden sm:flex items-center gap-2 text-zinc-400 text-xs flex-shrink-0 pr-1">
      ...
      {/* Editor Body */}
      <div className="relative w-full p-3 sm:p-4 md:p-6 overflow-x-auto">
        {children}
      </div>
    </div>
  );
